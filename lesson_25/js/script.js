...
Треба прокрутити скролу сторінки до кнопки */
...
function scrollToBlock(element) {
	if (element) {
		...
		return `Прокрутка до "${dataScrollPosition}" блоку "${element.tagName}" з класом: .${element.className}`
	}
	else {
		return `Помилка пошуку блоку для скролу`
	}
}

showMessage(scrollToBlock(elementButton))
